...
var widgets = (function () {
  var widgets = {
    ...
    addToCart: function (shop_id, product_id, price, obj) {
      ...
      $.post(
        ...
        function (data) {
          ...
          var result = true,
            amount_limit = amount;
          var has_amount_limit =
            data.product_amount_limit == "1" ? true : false;
          if (has_amount_limit) {
          ...
          if (
              ...
              if (cart_item_split.length == 2) {
                if (!found && cart_item_split[0] == product_id) {
                  ...
                  if (has_amount_limit && ta > amount_limit) {
                    ...
                    result = false;
                    widgets.msg(
                      window._s3Lang["JS_SHOP_PRODUCT_AMOUNT_TOTAL"] +
                        ": " +
                        amount_limit,
                      obj
                    );
                  ...
                  if (cart_new != "") cart_new = cart_new + ";";
                  cart_new =
                    cart_new + cart_item_split[0] + "=" + cart_item_split[1];
            ...
            if (result) widgets.msg(window._s3Lang["JS_ADDED"], obj);
            return true;
          }
        },
        "json"
    ...
    createPreloader: function (node) {
      ...
      with (div.style) {
        ...
        height = 32;
        background =
          "url(" + this.preloaderURL + ") center center no-repeat transparent";
        zIndex = 2000;
      }
      return div;
    },
  ...
  String.prototype.trim = function (s) {
    var d = s ? s : "[\\t\\s\\n\\r]";
    return this.replace(new RegExp("^" + d + "+"), "").replace(
      new RegExp(d + "+$"),
      ""
    );
  };
  ...
  if ("debug" in query || sessionStorage.getItem("csspatchDebug")) {
    loadScript(
      "//debug-frontend.oml.ru/g/s3/csspatch/csspatch.client.js?_=" + Date.now()
    );
    sessionStorage.setItem("csspatchDebug", 1);
  } else if (
    ("csspatch" in query || sessionStorage.getItem("csspatch")) &&
...
var A_TCALDEF = {
  ...
  close: window._s3Lang["JS_CLOSE_CALENDAR"] || "",
  open: window._s3Lang["JS_OPEN_CALENDAR"] || "",
};
var RU_TCALDEF = {
  months: [
    "Январь",
  ...
  if (n_day > d_numdays.getDate())
    return alert(
      "Invalid day of month value: '" +
        n_day +
        "'.\nAllowed range for selected month is 01 - " +
        d_numdays.getDate() +
        "."
    );
...
function tcal(a_cfg, a_tpl) {
  ...
  if (!this.e_icon) {
    ...
    divPlace.innerHTML =
      ...
      '" onclick="A_TCALS[\'' +
      this.s_id +
      '\'].f_toggle()" class="tcalIcon" alt="#JS_OPEN_CALENDAR#|s3_localize" />';
...
function f_tcalShow(d_date) {
  ...
  if (this.a_cfg.formname) {
    ...
    this.e_input = this.a_cfg.place.querySelector(
      'input[id="' + this.a_cfg.controlname + '"]'
    );
...
function f_tcalHide(n_date) {
  ...
  this.e_icon.title = window._s3Lang["JS_OPEN_CALENDAR"];
...
function f_tcalUpdate(d_date) {
  ...
  var d_today = this.a_cfg.today
    ? this.f_parseDate(this.a_cfg.today)
    : d_client;
  var d_selected =
    this.e_input.value == ""
      ? this.a_cfg.selected
        ? this.f_parseDate(this.a_cfg.selected)
        : d_today
  ...
  d_firstday.setDate(
    1 - ((7 + d_firstday.getDay() - this.a_tpl.weekstart) % 7)
  );
  var a_class,
    s_html =
      ...
      (this.a_tpl.yearscroll
        ? "<td" +
          this.f_relDate(d_date, -1, "y") +
          ' title="Previous Year"><img src="' +
          this.a_tpl.imgpath +
          'prev_year.gif" /></td>'
        : "") +
      "<td" +
      this.f_relDate(d_date, -1) +
      ' title="Previous Month"><img src="' +
      this.a_tpl.imgpath +
      'prev_mon.gif" /></td><th>' +
      ...
      "</th><td" +
      this.f_relDate(d_date, 1) +
      ' title="Next Month"><img src="' +
      this.a_tpl.imgpath +
      'next_mon.gif" /></td>' +
      (this.a_tpl.yearscroll
        ? "<td" +
          this.f_relDate(d_date, 1, "y") +
          ' title="Next Year"><img src="' +
          this.a_tpl.imgpath +
          'next_year.gif" /></td></td>'
        : "") +
  ...
  for (var i = 0; i < 7; i++)
    s_html +=
      "<th>" + this.a_tpl.weekdays[(this.a_tpl.weekstart + i) % 7] + "</th>";
  s_html += "</tr>";
  ...
  var n_left =
    f_getPosition(this.e_icon, "Left") - n_width + this.e_icon.offsetWidth;
...
var b_mac = s_userAgent.indexOf("mac") != -1,
  ...
  b_ie6 =
    s_userAgent.indexOf("msie 6") != -1 && s_userAgent.indexOf("opera") == -1;
